...
import { z } from 'zod';
import { ChainClient, KNOWN_TOKENS, apiPost } from '../config.js';

const COMMON_PAIRS: Array<{ inMint: string; outMint: string }> = [
  {
...
export function registerTokenTools(server: McpServer, chain: ChainClient) {
  server.tool(
    'byreal_token_price',
    'Get live USD price of a token available on Byreal. Uses the Router API — no API key needed.',
    {
    ...
    async ({ tokenSymbolOrMint }) => {
      ...
      for (const [addr, info] of Object.entries(KNOWN_TOKENS)) {
        if (info.symbol.toUpperCase() === upper) { mint = addr; break; }
      }
      ...
      if (mint === usdcMint) {
        return { content: [{ type: 'text' as const, text: `${symbol}: $1.000000 (USDC = USD)` }] };
      }

      const data = await apiPost<any>(ROUTER_URL, {
        inputMint: mint,
        outputMint: usdcMint,
  ...
  server.tool(
    'byreal_market_overview',
    'Get a quick market overview of key Byreal token prices. Fetches SOL, bbSOL prices via Router.',
    {},
    async () => {
      const usdcMint = 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v';
      const lines: string[] = ['📊 Byreal Market Overview\n'];
  ...
  server.tool(
    'byreal_known_tokens',
    'List all known Byreal tokens with mint addresses and decimals.',
    {},
    async () => {
      const lines = Object.entries(KNOWN_TOKENS).map(
        ([addr, info]) => `${info.symbol}: ${addr} (${info.decimals} decimals)`
      );
      return {
        content: [{
          type: 'text' as const,
