...
export function loadConfig() {
    ...
    if (process.env.PRIVY_APP_ID)
        envConfig.privyAppId = process.env.PRIVY_APP_ID;
    if (process.env.PRIVY_APP_SECRET)
        envConfig.privyAppSecret = process.env.PRIVY_APP_SECRET;
    if (process.env.RESEND_API_KEY)
        envConfig.resendApiKey = process.env.RESEND_API_KEY;
    if (process.env.RESEND_FROM)
        envConfig.resendFrom = process.env.RESEND_FROM;
    try {
        if (fs.existsSync(CONFIG_FILE)) {
            const file = JSON.parse(fs.readFileSync(CONFIG_FILE, 'utf-8'));
            return { ...file, ...envConfig };
        }
    }
    catch { }
...
async function privySignTransaction(config, walletId, unsignedTxB64) {
    ...
    if (d?.signed_transaction)
        return Buffer.from(d.signed_transaction, 'base64');
    if (d?.signedTransaction)
        return Buffer.from(d.signedTransaction, 'base64');
 ...
 */
export async function verifyAndCreateWallet(code) {
    const config = loadConfig();
    if (!config) {
 ...
 */
export async function signAndSend(unsignedTxB64) {
    const config = loadConfig();
    const wallet = loadWallet();
    if (!config)
        throw new Error('Wallet not configured');
    if (!wallet)
        throw new Error('No wallet found. Run byreal_wallet_setup first.');
...
export async function getWalletStatus() {
    ...
    const wallet = loadWallet();
    if (!config)
        return { configured: false, hasWallet: false };
    if (!wallet)
        return { configured: true, hasWallet: false };
    const result = {
        configured: true,
        hasWallet: true,
        address: wallet.address,
