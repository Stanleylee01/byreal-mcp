 ...
 */
function runSdkScript(script, env = {}) {
    const fullEnv = {
        ...process.env,
        ...env,
...
export function registerLiquidityTools(server, chain) {
    // ==================== Fee Collection (Server-side API) ====================
    server.tool('byreal_unclaimed_fees', 'Check unclaimed fees and rewards for a wallet. Shows amounts by token for each position.', {
        userAddress: z.string().describe('Wallet public key'),
    }, async ({ userAddress }) => {
        const data = await apiFetch(`${API_BASE}/dex/v2/position/unclaimed-v2`, { userAddress });
        if (!data?.list?.length && !data?.unclaimedFees?.length) {
            // Also check the old endpoint
            const data2 = await apiFetch(`${API_BASE}/dex/v2/position/unclaimed-data`, { userAddress });
            const fees = data2?.unclaimedFees ?? [];
            const incentives = data2?.unclaimedOpenIncentives ?? [];
            if (!fees.length && !incentives.length) {
                return { content: [{ type: 'text', text: 'No unclaimed fees or rewards.' }] };
            }
            const feeLines = fees.map((f) => `Fee: ${f.amount ?? f.tokenEstA ?? '?'} ${f.tokenSymbol ?? f.tokenAddress?.slice(0, 8) ?? '?'} ($${Number(f.price ?? 0) * Number(f.amount ?? 0)})`);
            const incLines = incentives.map((r) => `Reward: ${r.amount ?? r.totalTokenAmount ?? '?'} ${r.tokenSymbol ?? '?'}`);
            return {
                content: [{
                        type: 'text',
    ...
    server.tool('byreal_collect_fees_tx', 'Build unsigned transaction(s) to collect trading fees from LP positions. Returns base64-encoded transactions to sign.', {
        ...
        if (positionAddresses?.length)
            body.positionAddresses = positionAddresses;
        const data = await apiPost(`${API_BASE}/dex/v2/incentive/encode-fee`, body);
        if (!data?.length) {
            return {
                content: [{ type: 'text', text: 'No fees to collect (all positions already claimed).' }],
            };
        }
        const lines = data.map((item, i) => {
            const tokens = item.tokens ?? [];
            const tokenInfo = tokens.map((t) => `${t.symbol ?? '?'}: ${t.amount ?? '?'}`).join(', ');
            return [
        ...
        const txPayloads = data.map((item) => item.txPayload).filter(Boolean);
        return {
            content: [{
                    type: 'text',
                    text: [
    ...
    server.tool('byreal_claim_rewards_tx', 'Build unsigned transaction(s) to claim incentive rewards or bonus from LP positions.', {
        walletAddress: z.string().describe('Wallet public key'),
        ...
        const data = await apiPost(`${API_BASE}/dex/v2/incentive/encode-v3`, { walletAddress, positionAddresses, type });
        if (!data?.rewardEncodeItems?.length && !data?.orderCode) {
            return {
                content: [{ type: 'text', text: 'No rewards to claim for these positions.' }],
            };
        }
    ...
    server.tool('byreal_submit_claim', 'Submit signed reward/bonus claim transactions to Byreal.', {
        ...
        const data = await apiPost(`${API_BASE}/dex/v2/incentive/order-v3`, { orderCode, walletAddress, signedTxPayload });
        return {
            content: [{
                    type: 'text',
    ...
    server.tool('byreal_submit_liquidity_tx', 'Submit signed liquidity transaction (open/close/add/remove position) to Byreal for broadcasting.', {
        signedTransactions: z.array(z.string()).describe('Signed transaction(s) as base64'),
    }, async ({ signedTransactions }) => {
        const data = await apiPost(`${API_BASE}/dex/v2/liquidity/send`, { data: signedTransactions });
        return {
            content: [{
                    type: 'text',
    ...
    server.tool('byreal_create_position_info', 'Get all the information needed to create a new LP position — aligned tick prices, estimated token amounts, and fees. Does NOT build a transaction.', {
        ...
        const poolData = await apiFetch(`${API_BASE}/dex/v2/pools/details`, { poolAddress });
        if (!poolData) {
    ...
    server.tool('byreal_position_pnl', 'Get detailed PnL breakdown for a specific position — deposit, withdrawals, fees, rewards, bonus, and net PnL.', {
        positionAddress: z.string().describe('Position address'),
    }, async ({ positionAddress }) => {
        const data = await apiFetch(`${API_BASE}/dex/v2/position/detail`, { address: positionAddress });
        if (!data) {
            return { content: [{ type: 'text', text: `Position not found.` }], isError: true };
        }
    ...
    server.tool('byreal_open_position', 'Build an unsigned transaction to open a new CLMM LP position. Returns base64 tx to sign.', {
        poolAddress: z.string().describe('Pool address'),
        priceLower: z.string().describe('Lower price bound (token B per token A)'),
        priceUpper: z.string().describe('Upper price bound (token B per token A)'),
        baseToken: z.enum(['A', 'B']).describe('Which token to use as base (A or B)'),
        ...
        try {
            ...
            return { content: [{ type: 'text', text: `SDK Error: ${msg}` }], isError: true };
        }
    });
    server.tool('byreal_close_position', 'Build an unsigned transaction to close a CLMM position (remove all liquidity and close). Returns base64 tx to sign.', {
    ...
    server.tool('byreal_remove_liquidity', 'Build an unsigned transaction to remove partial liquidity from a CLMM position. Returns base64 tx to sign.', {
        nftMint: z.string().describe('Position NFT mint address'),
    ...
    server.tool('byreal_copy_position', 'Copy a top farmer\'s position: look up an existing position by address and build an unsigned tx to replicate it for your wallet. Returns base64 tx to sign.', {
        ...
        let posDetail;
        try {
            posDetail = await apiFetch(`${API_BASE}/dex/v2/position/detail`, { address: positionAddress });
        }
        catch (e) {
            return { content: [{ type: 'text', text: `Failed to fetch position: ${e.message}` }], isError: true };
        }
        if (!posDetail?.pool) {
            return { content: [{ type: 'text', text: 'Position not found or has no pool data.' }], isError: true };
        }
        ...
        if (tickLower === undefined || tickUpper === undefined || !poolAddress) {
            return {
                content: [{ type: 'text', text: `Position ${positionAddress} missing tick/pool data. Lower: ${tickLower}, Upper: ${tickUpper}, Pool: ${poolAddress}` }],
                isError: true,
            };
        }
