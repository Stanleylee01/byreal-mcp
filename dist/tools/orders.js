...
export function registerOrderTools(server, chain) {
    server.tool('byreal_order_history', 'Get swap/trade order history for a wallet on Byreal.', {
        userAddress: z.string().describe('Wallet public key'),
        page: z.number().min(1).default(1),
        pageSize: z.number().min(1).max(50).default(10),
    }, async ({ userAddress, page, pageSize }) => {
        const data = await apiFetch(`${API_BASE}/dex/v2/order/list`, {
            userAddress, page: String(page), pageSize: String(pageSize),
        });
        const records = data?.records ?? data?.list ?? [];
        if (!records.length) {
            return { content: [{ type: 'text', text: `No orders found for ${userAddress}` }] };
        }
        const lines = records.map((o, i) => {
    ...
    server.tool('byreal_position_detail', 'Get detailed info for a specific LP position by its address.', {
        address: z.string().describe('Position address (positionAddress from list)'),
    }, async ({ address }) => {
        const data = await apiFetch(`${API_BASE}/dex/v2/position/detail`, { address });
        if (!data) {
            return { content: [{ type: 'text', text: `Position ${address} not found.` }], isError: true };
        }
        const lines = [
    ...
    server.tool('byreal_position_overview', 'Get position overview summary for a wallet — total count and aggregated stats.', {
        userAddress: z.string().describe('Wallet public key'),
    }, async ({ userAddress }) => {
        const data = await apiFetch(`${API_BASE}/dex/v2/position/overview`, { userAddress });
        return {
            content: [{
                    type: 'text',
                    text: [
