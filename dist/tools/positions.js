...
export function registerPositionTools(server, chain) {
    server.tool('byreal_list_positions', 'List all CLMM liquidity positions for a wallet on Byreal. Shows pool pair, PnL, earned fees, deposit, age, and status.', {
        ...
        const data = await apiFetch(API_ENDPOINTS.MY_POSITIONS, {
            userAddress: walletAddress,
            page: String(page), pageSize: String(pageSize),
        });
        if (!data?.positions?.length) {
            return {
                content: [{ type: 'text', text: `No positions found for ${walletAddress}` }],
            };
        }
        const lines = data.positions.map((p, i) => {
        ...
        // Summary
        const totalDeposit = data.positions.reduce((s, p) => s + Number(p.totalDeposit || 0), 0);
        const totalPnl = data.positions.reduce((s, p) => s + Number(p.pnlUsd || 0), 0);
        const totalEarned = data.positions.reduce((s, p) => s + Number(p.earnedUsd || 0), 0);
        const summary = [
    ...
    server.tool('byreal_calculate_apr', 'Estimate APR for a hypothetical position on a Byreal pool. Uses pool volume and fee data.', {
