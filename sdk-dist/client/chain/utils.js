...
import { LiquidityMath, PoolUtils, SqrtPriceMath, TickMath, } from '../../instructions/index.js';
 ...
 */
export function alignPriceToTickPrice(price, poolInfo) {
    const { price: roundedPrice } = TickMath.getTickAlignedPriceDetails(price, poolInfo.tickSpacing, poolInfo.mintDecimalsA, poolInfo.mintDecimalsB);
    return roundedPrice;
}
/**
...
export function getAmountBFromAmountA(params) {
    ...
    const sqrtPriceX64A = SqrtPriceMath.priceToSqrtPriceX64(priceLowerDecimal, poolInfo.mintDecimalsA, poolInfo.mintDecimalsB);
    const sqrtPriceX64B = SqrtPriceMath.priceToSqrtPriceX64(priceUpperDecimal, poolInfo.mintDecimalsA, poolInfo.mintDecimalsB);
...
export function calculateApr(params) {
    ...
    const { volume24h, feeRate, positionUsdValue, amountA, amountB, tickLower, tickUpper, poolInfo, scene = 'exist', existLiquidity = new BN(0), } = params;
...
export function calculateRewardApr(params) {
    const { reward24hUsdValue, positionUsdValue, amountA, amountB, tickLower, tickUpper, poolInfo, scene = 'exist', existLiquidity = new BN(0), } = params;
    return _calculateApr({
...
export function _calculateApr(params) {
    const { fee24hUsdValue, positionUsdValue, amountA, amountB, tickLower, tickUpper, poolInfo, scene = 'exist', existLiquidity = new BN(0), } = params;
    ...
    const userActiveLiquidity = LiquidityMath.getLiquidityFromTokenAmounts(sqrtPriceCurrentX64, sqrtPriceLowerX64, sqrtPriceUpperX64, amountA, amountB);
...
export function calculateRangeAprs(params) {
    ...
    for (const range of percentRanges) {
        ...
        if (range === -1) {
            ...
            tickLower = TickMath.getTickWithPriceAndTickspacing(lowerPrice, poolInfo.tickSpacing, poolInfo.mintDecimalsA, poolInfo.mintDecimalsB);
            tickUpper = TickMath.getTickWithPriceAndTickspacing(upperPrice, poolInfo.tickSpacing, poolInfo.mintDecimalsA, poolInfo.mintDecimalsB);
        }
        ...
        const { amountA, amountB } = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceLowerX64, sqrtPriceUpperX64, liquidityToUse, false // Do not round up
        );
 ...
 */
export async function getTokenProgramId(connection, mintAddress) {
    try {
