import { TOKEN_PROGRAM_ID, TOKEN_2022_PROGRAM_ID, getAssociatedTokenAddress, getMint, AccountLayout, } from '@solana/spl-token';
...
export class Token {
     ...
     */
    async detectTokenTypeAndGetBalance(walletAddress, tokenMintAddress) {
        ...
        try {
            ...
            const tokenAccount = await getAssociatedTokenAddress(mintPublicKey, walletPublicKey, false, TOKEN_2022_PROGRAM_ID);
     ...
     */
    async batchDetectTokenTypeAndGetBalance(walletAddress, tokenMintAddresses) {
        // Concurrent processing of all tokens
        const results = await Promise.all(tokenMintAddresses.map(async (tokenMintAddress) => {
