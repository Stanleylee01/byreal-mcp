...
import { FEE_RATE_DENOMINATOR, MAX_SQRT_PRICE_X64, MAX_TICK, MIN_SQRT_PRICE_X64, MIN_TICK, NEGATIVE_ONE, ONE, ZERO, } from '../constants.js';
...
export class SwapMath {
     ...
     */
    static swapCompute(programId, poolId, tickArrayInfo, tickArrayBitmap, tickarrayBitmapExtension, zeroForOne, fee, liquidity, currentTick, tickSpacing, currentSqrtPriceX64, amountSpecified, lastSavedTickArrayStartIndex, sqrtPriceLimitX64, catchLiquidityInsufficient = false) {
        ...
        if (!tickArrayCurrent) {
            throw new Error(`TickArray not found in cache for startIndex: ${lastSavedTickArrayStartIndex}. Please ensure tickArrayCache is properly initialized.`);
        }
        let loopCount = 0;
        ...
        // state.tick > MIN_TICK
            ...
            if (!nextInitTick?.liquidityGross.gtn(0)) {
                ...
                if (!tickArrayCurrent) {
                    throw new Error(`TickArray not found in cache for startIndex: ${tickAarrayStartIndex}. Missing tick array data during swap computation.`);
                }
                try {
                    nextInitTick = TickUtils.firstInitializedTick(tickArrayCurrent, zeroForOne);
                }
                catch (error) {
                    throw new Error(`Failed to find initialized tick in array ${tickAarrayStartIndex}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
    ...
    static swapStepCompute(sqrtPriceX64Current, sqrtPriceX64Target, liquidity, amountRemaining, feeRate, // Base 1000000
    zeroForOne) {
        const swapStep = {
        ...
        if (baseInput) {
            const amountRemainingSubtractFee = MathUtils.mulDivFloor(amountRemaining, FEE_RATE_DENOMINATOR.sub(new BN(feeRate.toString())), FEE_RATE_DENOMINATOR);
        ...
        if (baseInput && !swapStep.sqrtPriceX64Next.eq(sqrtPriceX64Target)) {
            ...
            swapStep.feeAmount = MathUtils.mulDivCeil(swapStep.amountIn, new BN(feeRate), FEE_RATE_DENOMINATOR.sub(new BN(feeRate)));
        }
