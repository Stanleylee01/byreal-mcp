...
export class TickArrayUtils {
     ...
     */
    static FIXED_TICK_ARRAY_DISCRIMINATOR = Buffer.from('c09b55cd31f9812a', 'hex');
    static DYN_TICK_ARRAY_DISCRIMINATOR = Buffer.from('6a8b98247599b838', 'hex');
    /**
     * 识别 Tick Array 的类型
     * @param accountData 账户数据
     * @returns 'Fixed' 或 'Dynamic'
     */
    static identifyTickArrayType(accountData) {
        ...
        if (discriminator.equals(this.FIXED_TICK_ARRAY_DISCRIMINATOR)) {
            ...
            throw new Error(`Unknown tick array discriminator: ${discriminator.toString('hex')}. ` +
     ...
     */
    static decodeDynTickArray(accountData, address) {
        // DynTickArrayState Header 长度：8 (discriminator) + 208 (struct) = 216 bytes
        if (accountData.length < DYN_TICK_ARRAY_HEADER_LEN) {
            throw new Error(`Invalid dyn tick array data: expected at least ${DYN_TICK_ARRAY_HEADER_LEN} bytes, got ${accountData.length}`);
        }
        // 1. 解析 header
     ...
     */
    static getTickStateFromContainer(container, tickIndex, tickSpacing) {
     ...
     * @returns Tick 或 null（如果未分配）
     */
    static getTickStateFromDynArray(dynTickArray, tickIndex, tickSpacing) {
        // 1. 计算逻辑偏移 (0-59)
        ...
        if (actualIndex >= dynTickArray.ticks.length) {
            throw new Error(`Tick state index out of bounds: ${actualIndex} >= ${dynTickArray.ticks.length}. ` +
     ...
     * @private
     */
    static parseTickArrayContainer(accountData, address) {
            ...
            return {
                ...
                data: {
                    ...
                    ticks: decoded.ticks.map((tick) => ({
                        ...
                        rewardGrowthsOutsideX64: tick.rewardGrowthsOutsideX64.map((r) => new BN(r.toString())),
                    })),
