...
describe('LiquidityMath.getLiquidityFromTokenAmounts', () => {
    test('current price is below the lower limit - only consider tokenA', () => {
        ...
        const liquidity = LiquidityMath.getLiquidityFromTokenAmounts(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, amountA, amountB);
        // verify the result
    ...
    test('price range order adjustment - ensure A is always less than B', () => {
        ...
        const expectedLiquidity = LiquidityMath.getLiquidityFromTokenAmounts(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, amountA, amountB);
        // call in the reversed order
        const liquidity = LiquidityMath.getLiquidityFromTokenAmounts(sqrtPriceCurrentX64, sqrtPriceX64B, sqrtPriceX64A, amountA, amountB);
...
describe('LiquidityMath.getAmountsFromLiquidity', () => {
    test('current price is below the lower limit - only return tokenA', () => {
        ...
        const expectedAmountA = LiquidityMath.getTokenAmountAFromLiquidity(sqrtPriceX64A, sqrtPriceX64B, liquidity, roundUp);
    ...
    test('current price is in the range - return two tokens', () => {
        ...
        const expectedAmountA = LiquidityMath.getTokenAmountAFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64B, liquidity, roundUp);
        const expectedAmountB = LiquidityMath.getTokenAmountBFromLiquidity(sqrtPriceX64A, sqrtPriceCurrentX64, liquidity, roundUp);
        const { amountA, amountB } = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, roundUp);
        // verify the result
        expect(amountA.toString()).toBe(expectedAmountA.toString());
    ...
    test('current price is above the upper limit - only return tokenB', () => {
        ...
        const expectedAmountB = LiquidityMath.getTokenAmountBFromLiquidity(sqrtPriceX64A, sqrtPriceX64B, liquidity, roundUp);
        const { amountA, amountB } = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, roundUp);
        // verify the result
    ...
    test('rounding behavior test - roundUp = true vs false', () => {
        ...
        const liquidity = new BN('10000000');
        // use roundUp = true
        const resultRoundUp = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, true);
        // use roundUp = false
        const resultRoundDown = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, false);
...
describe('LiquidityMath.getAmountBFromAmountA', () => {
    test('calculate tokenB from tokenA - normal case', () => {
        ...
        const amountA = new BN('1000000');
        const amountB = LiquidityMath.getAmountBFromAmountA(startSqrtPriceX64, endSqrtPriceX64, currentSqrtPriceX64, amountA);
        ...
        const expectedAmountB = LiquidityMath.getTokenAmountBFromLiquidity(startSqrtPriceX64, currentSqrtPriceX64, liquidity, true);
        // verify the result
        expect(amountB.toString()).toBe(expectedAmountB.toString());
    });
    ...
    test('price range order adjustment - ensure start is always less than end', () => {
        ...
        const amountA = new BN('1000000');
        // call in the correct order
        const expectedAmountB = LiquidityMath.getAmountBFromAmountA(endSqrtPriceX64, startSqrtPriceX64, currentSqrtPriceX64, amountA);
        // call in the reversed order
        const amountB = LiquidityMath.getAmountBFromAmountA(startSqrtPriceX64, endSqrtPriceX64, currentSqrtPriceX64, amountA);
        // verify the result is the same
        expect(amountB.toString()).toBe(expectedAmountB.toString());
...
describe('LiquidityMath.getAmountAFromAmountB', () => {
    test('calculate tokenA from tokenB - normal case', () => {
        ...
        const endSqrtPriceX64 = new BN('300000');
        const amountB = new BN('500000');
        const amountA = LiquidityMath.getAmountAFromAmountB(startSqrtPriceX64, endSqrtPriceX64, currentSqrtPriceX64, amountB);
        // verify the correctness of the calculation process
        // 1. calculate the liquidity
        ...
        const expectedAmountA = LiquidityMath.getTokenAmountAFromLiquidity(currentSqrtPriceX64, endSqrtPriceX64, liquidity, true);
        // verify the result
        expect(amountA.toString()).toBe(expectedAmountA.toString());
    });
    test('current price is below the lower limit - return 0', () => {
    ...
    test('price range order adjustment - ensure start is always less than end', () => {
        ...
        const amountB = new BN('500000');
        // call in the correct order
        const expectedAmountA = LiquidityMath.getAmountAFromAmountB(endSqrtPriceX64, startSqrtPriceX64, currentSqrtPriceX64, amountB);
        // call in the reversed order
        const amountA = LiquidityMath.getAmountAFromAmountB(startSqrtPriceX64, endSqrtPriceX64, currentSqrtPriceX64, amountB);
        // verify the result is the same
        expect(amountA.toString()).toBe(expectedAmountA.toString());
...
describe('LiquidityMath consistency test', () => {
    test('liquidity calculation and token quantity calculation consistency', () => {
        ...
        const { amountA: calculatedAmountA, amountB: calculatedAmountB } = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, false // do not round up for comparison
        );
    ...
    test('consistency test between getAmountBFromAmountA and getAmountAFromAmountB', () => {
        ...
        // 1. A -> B
        const calculatedAmountB = LiquidityMath.getAmountBFromAmountA(startSqrtPriceX64, endSqrtPriceX64, currentSqrtPriceX64, initialAmountA);
        // 2. B -> A
        const calculatedAmountA = LiquidityMath.getAmountAFromAmountB(startSqrtPriceX64, endSqrtPriceX64, currentSqrtPriceX64, calculatedAmountB);
        ...
        const relativeError = Math.abs(Number(initialAmountA.toString()) - Number(calculatedAmountA.toString())) /
...
describe('LiquidityMath boundary and special case test', () => {
    test('zero input test', () => {
        ...
        const { amountA, amountB } = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, zeroLiquidity, true);
        expect(amountA.toString()).toBe('0');
        expect(amountB.toString()).toBe('0');
        // zero token quantity
        const zeroAmount = new BN(0);
        const amountAResult = LiquidityMath.getAmountAFromAmountB(sqrtPriceX64A, sqrtPriceX64B, sqrtPriceCurrentX64, zeroAmount);
        const amountBResult = LiquidityMath.getAmountBFromAmountA(sqrtPriceX64A, sqrtPriceX64B, sqrtPriceCurrentX64, zeroAmount);
    ...
    test('extreme liquidity value test', () => {
        ...
        const { amountA, amountB } = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, largeLiquidity, true);
    ...
    test('equal price point test', () => {
        ...
        const { amountA, amountB } = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64, sqrtPriceX64, liquidity, true);
    ...
    test('price point near current price test', () => {
        ...
        const liquidity = new BN('10000000');
        // call the function
        const result = LiquidityMath.getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, true);
        // verify the result
