...
export async function makeTransaction(params) {
    ...
    if (addComputeBudget) {
        const computeUnitLimit = options.computeUnitLimit || (await estimateComputeUnits(connection, instructions, payerPublicKey, blockhash));
        ...
        finalInstructions.push(ComputeBudgetProgram.setComputeUnitLimit({ units: computeUnitLimit }), ComputeBudgetProgram.setComputeUnitPrice({ microLamports: finalComputeUnitPrice }));
    }
...
export async function sendTransaction(params) {
    ...
    // Set default options
    const { skipPreflight = false, preflightCommitment = 'confirmed', maxRetries = 3, confirmationTimeout = 30000, confirmationRetryInterval = 2000, confirmationRetries = 10, } = options;
 ...
 */
async function confirmTransactionWithOptimizedPolling(connection, txid, options) {
