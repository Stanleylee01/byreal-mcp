 ...
 */
export async function estimateComputeUnits(connection, instructions, payerPublicKey, _blockhash) {
    try {
        ...
        if (simulation.value.logs && simulation.value.unitsConsumed) {
            ...
            const estimatedUnits = Math.min(Math.max(consumedUnits + 100_000, // 100k buffer for safety
