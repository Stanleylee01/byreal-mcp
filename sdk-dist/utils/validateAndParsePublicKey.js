...
export function tryParsePublicKey(v) {
    try {
...
export function solToWSol(mint) {
 ...
 */
export function validateAndParsePublicKey({ publicKey: orgPubKey, solToWSol, }) {
    const publicKey = tryParsePublicKey(orgPubKey.toString());
    if (publicKey instanceof PublicKey) {
        if (solToWSol && publicKey.equals(SOLMint))
            return WSOLMint;
        return publicKey;
    }
    if (solToWSol && publicKey.toString() === SOLMint.toBase58())
        return WSOLMint;
